...
lang UtestLoader = MCoreLoader + GenerateEqLoader + GeneratePprintLoader + StripUtestLoader
  ...
  | UtestHook
    ...
    , runner : Name
    , exitOnFailure : Name
    , includeStaticUtestIf : Info -> Bool
    }

  -- Enable code generation replacing `utest` with equivalent
  -- code. Will remove `StripUtestHook` if present.
  sem enableUtestGeneration : (Info -> Bool) -> Loader -> Loader
  sem enableUtestGeneration includeStaticUtestIf = | loader ->
    ...
    let hook =
      ...
      , includeStaticUtestIf = includeStaticUtestIf
      } in
  ...
  sem _postTypecheck loader decl = | UtestHook hook ->
    match decl with DeclUtest d then
      if hook.includeStaticUtestIf d.info then
  ...
  | TmUtest x ->
    if or (not static) (hook.includeStaticUtestIf x.info) then
