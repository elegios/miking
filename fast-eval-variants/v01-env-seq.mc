-- ENVIRONMENT: builtin sequence instead of `List`.
--
-- Changes the environment from the `List` ADT in list.mc to MCore's builtin
-- sequence type, consed onto the front and searched with a sequence pattern.
-- Builtin sequences are ropes, so `cons` is cheap but every `[x] ++ rest` match
-- has to look through the rope structure; `List` is a plain linked list where
-- the same step is a single constructor match.
...
lang EvalF = Ast
  ...
  type EvalFEnv = [(Int, Val)]

  sem evalFEnvLookup : Int -> EvalFEnv -> Val
  sem evalFEnvLookup s1 =
  | [] -> error "env lookup failed!"
  | [(s2, val)] ++ env -> if eqi s1 s2 then val else evalFEnvLookup s1 env
