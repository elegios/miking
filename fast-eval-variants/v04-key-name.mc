-- KEYS: `Name` in the environment instead of `Int`.
--
-- Skips `nameGetSym` entirely and stores the whole `Name` (a string paired with
-- a symbol), comparing with `nameEq`.  This is what an evaluator that does not
-- want to depend on symbolization having run would have to do: `nameEq` checks
-- whether both names carry symbols before comparing them, and falls back to
-- comparing strings when they do not.
...
lang EvalF = Ast
  ...
  type EvalFEnv = List (Name, Val)

  sem evalFEnvLookup : Name -> EvalFEnv -> Val
  sem evalFEnvLookup s1 =
  | Nil _ -> error "env lookup failed!"
  | Cons ((s2, val), env) ->
    if nameEq s1 s2 then val else evalFEnvLookup s1 env
...
lang VarEvalF = EvalF + VarAst
  sem mkEvalF =
  | TmVar r -> evalFEnvLookup r.ident
end
...
lang NamedPatEvalF = MatchEvalF + NamedPat
  ...
  | PatNamed {ident = PName name} ->
    lam val. lam env. Some (Cons ((name, val), env))
  ...
  | PatSeqEdge r ->
    ...
    let middle =
      match r.middle with PName name then
        lam vals. lam env. Some (Cons ((name, VSeq vals), env))
