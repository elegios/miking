...
lang LetEvalF = EvalF + LetDeclAst
  ...
  | DeclLet r ->
    match nameGetSym r.ident with Some s then
      ...
      -- `lowerAll` emits a few discarded bindings with no name at all (an
      -- empty, unsymbolized ident).  Nothing can refer to one, so evaluate the
      -- body for its effect and extend nothing; a variable that did refer to
      -- such a name would still fail loudly in VarEvalF.
      let body = mkEvalF r.body in
      lam env. body env; env
end
...
lang IOEvalF =
  ConstEvalF + SeqEvalF + CharEvalF + RecordEvalF + IOAst

  -- Needed only because `lowerAll` builds an error message with `print` on the
  -- fallthrough path of every lowered match.  `mkEvalF` compiles the whole AST
  -- up front, so the constant has to resolve even though nothing reaches it in
  -- a program whose matches are exhaustive.  It therefore costs nothing at run
  -- time and does not affect what this variant measures.
  sem valSeqToString : Val -> String
  sem valSeqToString =
  | VSeq vals -> map (lam v. match v with VChar c in c) vals
  | _ -> error "Expected a string in IOEvalF!"

  sem mkDeltaF =
  | c & CPrint _ -> VConst1
    (c , lam s. print (valSeqToString s); VRecord (mapEmpty cmpSID))
  | c & CPrintError _ -> VConst1
    (c , lam s. printError (valSeqToString s); VRecord (mapEmpty cmpSID))
  | c & CDPrint _ -> VConst1
    (c , lam. VRecord (mapEmpty cmpSID))
...
lang MExprEvalF =
  ...
  FloatIntConversionEvalF + SeqOpEvalF + IOEvalF + SysEvalF +
