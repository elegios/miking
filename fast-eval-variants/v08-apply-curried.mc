-- APPLICATION: curry `applyF` instead of passing a pair.
--
-- `sem` can only match on one argument, so the baseline packs the function and
-- its argument into a tuple to dispatch on the function.  That allocates a
-- two-word block for every application in the program.  MLang sems accept extra
-- parameters ahead of the matched one, so the argument can be passed beside the
-- scrutinee instead and the tuple disappears.
--
-- Measured over this suite the difference is not detectable -- the allocation is
-- real, but OCaml's minor heap makes it free at this granularity.  The variant is
-- kept because that is a result worth being able to re-check, not because it is a
-- win.
...
lang AppEvalF = EvalF + AppAst + ConstAst + UnknownTypeAst
  ...
  | r ->
    ...
    lam env. applyF (rhs env) (lhs env)
...
lang ConstEvalF = AppEvalF + ConstAst + UnknownTypeAst
  ...
  sem applyF val =
  | VConst1 (_, f) -> f val
  | VConst2 (c, f) -> VConst1 (c, f val)
  | VConst3 (c, f) -> VConst2 (c, f val)
...
lang SeqOpEvalF =
  ...
  | c & CMap _ -> VConst2
    (c , lam f. lam s.
      match s with VSeq s in VSeq (map (lam x. applyF x f) s))
  | c & CMapi _ -> VConst2
    (c , lam f. lam s.
      match s with VSeq s in
      VSeq (mapi (lam i. lam x. applyF x (applyF (VInt i) f)) s))
  | c & CIter _ -> VConst2
    (c , lam f. lam s.
      match s with VSeq s in
      iter (lam x. applyF x f; ()) s;
      VRecord (mapEmpty cmpSID))
  | c & CIteri _ -> VConst2
    (c , lam f. lam s.
      match s with VSeq s in
      iteri (lam i. lam x. applyF x (applyF (VInt i) f); ()) s;
      VRecord (mapEmpty cmpSID))
  | c & CCreate _ -> VConst2
    (c , lam n. lam f.
      match n with VInt n in VSeq (create n (lam i. applyF (VInt i) f)))
  | c & CCreateList _ -> VConst2
    (c , lam n. lam f.
      match n with VInt n in VSeq (createList n (lam i. applyF (VInt i) f)))
  | c & CCreateRope _ -> VConst2
    (c , lam n. lam f.
      match n with VInt n in VSeq (createRope n (lam i. applyF (VInt i) f)))
  | c & CFoldl _ -> VConst3
    (c , lam f. lam acc. lam s.
      match s with VSeq s in
      foldl (lam acc. lam x. applyF x (applyF acc f)) acc s)
  | c & CFoldr _ -> VConst3
    (c , lam f. lam acc. lam s.
      match s with VSeq s in
      foldr (lam x. lam acc. applyF acc (applyF x f)) acc s)
end
