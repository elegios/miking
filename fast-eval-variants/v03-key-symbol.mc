-- KEYS: `Symbol` in the environment instead of `Int`.
--
-- The baseline hashes each name's symbol to an `Int` once, at build time, and
-- compares with `eqi`.  This variant keeps the `Symbol` itself and compares with
-- `eqsym`.  It measures what the `sym2hash` step buys: whether comparing symbols
-- costs more than comparing the machine integers they hash to.
...
lang EvalF = Ast
  ...
  type EvalFEnv = List (Symbol, Val)

  sem evalFEnvLookup : Symbol -> EvalFEnv -> Val
  sem evalFEnvLookup s1 =
  | Nil _ -> error "env lookup failed!"
  | Cons ((s2, val), env) ->
    if eqsym s1 s2 then val else evalFEnvLookup s1 env
