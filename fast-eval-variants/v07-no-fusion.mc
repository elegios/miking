-- REVERSION: drop the fusion of saturated constant applications.
--
-- The baseline recognises a constant applied to as many arguments as it takes and
-- calls the delta function directly.  This variant removes that and sends every
-- application through `applyF`, which is what the evaluator did before the
-- optimisation was folded in.
--
-- It is kept as a reversion rather than deleted so the suite keeps a standing
-- check that the optimisation still earns its keep as the evaluator grows --
-- a variant that is a pure loss today can stop being one.
...
lang AppEvalF = EvalF + AppAst + ConstAst + UnknownTypeAst
  ...
  -- Every application goes through `applyF`, including `addi x y`, which
  -- therefore builds a VConst2, applies it to get a VConst1, and takes that
  -- apart again -- on every evaluation.
  sem mkEvalF =
  | TmApp r ->
