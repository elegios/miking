-- ENVIRONMENT: `Map Int Val` instead of a linear list.
--
-- Trades O(n) lookup for O(log n), at the cost of rebuilding part of a balanced
-- tree on every binding rather than allocating one cons cell.  Since binding is
-- far more frequent than deep lookup in most programs, this is the interesting
-- trade to measure -- see the env-lookup-shallow/deep benchmark pair.
...
lang EvalF = Ast
  ...
  type EvalFEnv = Map Int Val

  sem evalFEnvLookup : Int -> EvalFEnv -> Val
  sem evalFEnvLookup s1 =
  | env ->
    match mapLookup s1 env with Some val then val
    else error "env lookup failed!"
...
lang LamEvalF = AppEvalF + LamAst
  ...
  | TmLam r ->
    match nameGetSym r.ident with Some s then
      ...
      lam env. VCls (lam val. body (mapInsert s (val) env))
...
lang LetEvalF = EvalF + LetDeclAst
  ...
  | DeclLet r ->
    match nameGetSym r.ident with Some s then
      let s = sym2hash s in
      let body = mkEvalF r.body in
      lam env. (mapInsert s (body env) env)
        ...
        (lam b.
          match b.body with TmLam r then
            match (nameGetSym b.ident, nameGetSym r.ident) with
              ...
              let body = mkEvalF r.body in
              (s1, lam env. lam val. body (mapInsert s2 (val) env))
    ...
    recursive let reclet = lam env.
      foldl
        (lam acc. lam t.
          ...
          (mapInsert s (VCls (lam val. cls (reclet env) val)) acc))
        env ts
    in
    reclet
end
...
lang NamedPatEvalF = MatchEvalF + NamedPat
  ...
  | PatNamed {ident = PName name} ->
    match nameGetSym name with Some s then
      let s = sym2hash s in
      lam val. lam env. Some (mapInsert s (val) env)
  ...
  | PatSeqEdge r ->
    ...
    let middle =
      match r.middle with PName name then
        match nameGetSym name with Some s then
          let s = sym2hash s in
          lam vals. lam env. Some (mapInsert s (VSeq vals) env)
...
match argv with [_, file] ++ _ then
  ...
  let eval = mkEvalF ast in eval (mapEmpty subi);
  ()
else
